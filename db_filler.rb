...
def get_recipes(url:)
  ...
  raise "api_key_used" unless response.kind_of? Net::HTTPSuccess
  response.read_body
end
...
spoonacular_api_keys.each do |spoonacular_api_key|
  begin
    loop do
      spoonacular_url = URI("https://api.spoonacular.com/recipes/random?apiKey=#{spoonacular_api_key}&number=100")

        recipes = get_recipes(url: spoonacular_url)

      myLittleChef_server_IP = config['myLittleChef_server_IP']
      post_recipes(url: myLittleChef_server_IP, recipes: recipes)
    end
